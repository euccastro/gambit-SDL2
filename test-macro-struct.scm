...
(define (test)
  (let* ((v (make-vec2))
         ...
         (p (make-vec2-and-pointer))
         (pv (vec2-and-pointer-v p))
         (pvp (vec2-and-pointer-vp p))
         (n (make-vec2-node))
         (n* (vec2-node-pointer n))
         (nv (vec2-node-v n))
         (nn (vec2-node-lets-call-this-scheme-next-just-for-the-sake-of-testing n))
         (n2 (make-vec2-node))
         (n2* (vec2-node-pointer n2))
         (n2v (vec2-node-v n2))
         (n2n (vec2-node-lets-call-this-scheme-next-just-for-the-sake-of-testing n2)))
    (vec2-x-set! v 5.0)
    (vec2-y-set! v 10.0)
    ...
    (println "vs si ((" (vec2-x vssa) "," (vec2-y vssa)
             "), (" (vec2-x vssb) "," (vec2-y vssb) ").")
    (vec2-and-pointer-vp-set! p v*)
    (println "pvp " (vec2-x (pointer->vec2 (vec2-and-pointer-vp p))))
    (vec2-x-set! nv 33.)
    (vec2-y-set! nv 44.)
